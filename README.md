# University-Empirical-Project

This project is an early work in progress and aims to answer the following research question: 

Can oil price movements with regards to the Iran war predict next day financial news sentiment?

#Important note on time to run and laptop specs

Depending on the specs of your device, the NLP models may take a couple of minutes to run. I am running an RTX 4070 super (desktop). However this still runs fine on my laptop (I5 ultra), where both scripts will complete after a total of 10min.
(Time to run should now be very feasible on any laptop, although more powerfull ones with accelerate it substancially).

Both contributors are me (Harrison Maxwell); one is an old account and the other is a new one using my student email. Im not sure why both accounts are listed as contributers as to my knowledge I have only pushed frmo my uni account.
