...
|--------|--------|--------|--------|
| const  | -0.6406 | 0.002 | no |
| Lagged_wti   | 0.0044 | 0.034 | yes |
| lagged_vix | -0.0025 | 0.681 | no|
| lagged_sentiment_mean | -0.0800 | 0.534 | no|

: **Naive regression results (Figure 6):** R^2^: 0.081 Adj. R^2^: 0.032 {.striped .hover}


The naive model fails. The R^2^ of 8% collapses to 3% when using adjusted R^2^, reflecting that we explain little of the variation in daily average sentiment. Additionally, only lagged_wti is statistically significant, with a 1 dollar increase in yesterday’s oil price would decrease sentiment by 0.0044. Clearly, oil prices alone do not drive news sentiment, there is far greater depth.
...
| **Item**  | **Coef**  | **P>[z]** | **Significant?** |
|--------|--------|--------|--------|
| const  | -0.6178 | 0.000 | yes |
| Lagged_wti   | 0.0033 | 0.017 | yes |
| High_vix_dummy | -0.0204 | 0.444 | no |
| Disagreement | 0.0028 | 0.000 | yes |
| Momentum | 0.4624 | 0.000 | yes |

: **Improved regression results (Figure 7):** R^2^: 0.663 Adj. R^2^: 0.638 {.striped .hover}

This model increases our R^2^ to 66% and our adjusted R^2^ to 64%, a significant improvement. The model is also highly jointly significant.

Momentum is statistically significant and has a coefficient of 0.4624. Thus, yesterday’s sentiment is a strong predictor of today’s, which makes sense as the conflict was ongoing. Disagreement was also statistically significant, with a coefficient of 0.0028. When journalists have conflicting views, the sentiment tends to be more positive, whereas if journalists agree, the sentiment is more negative. This does make sense intuitively, as a clear negative event gives less room for speculation.
...
::: {.callout-note}
For the purposes of marking, this blog is around 2414 words long, including words in tables, figures, and headings.
:::
